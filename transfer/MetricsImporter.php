...
class MetricsImporter
{
    ...
    private FullJournalImportExportDeployment $deployment;

    public function __construct(FullJournalImportExportDeployment $deployment)
    {
        $this->deployment = $deployment;
    }

    public function import(DOMElement $root): void
    {
    ...
    private function importContextMetric(DOMElement $node): void
    {
        $key = [
            ...
            'context_id' => (int) $this->deployment->getContext()->getId(),
            'date' => $this->date($node),
        ];
    ...
    private function importSubmissionMetric(DOMElement $node): void
    {
        $key = [
            'load_id' => $this->required($node, 'load_id'),
            'context_id' => (int) $this->deployment->getContext()->getId(),
            'submission_id' => $this->deployment->requireReference(
                'submission',
    ...
    private function importIssueMetric(DOMElement $node): void
    {
        $key = [
            'load_id' => $this->required($node, 'load_id'),
            'context_id' => (int) $this->deployment->getContext()->getId(),
            'issue_id' => $this->deployment->requireReference('issue', $this->required($node, 'issue_ref')),
    ...
    private function destinationInstitutionId(DOMElement $node, string $granularity): ?int
    {
        ...
        foreach (DB::table('institutions')
            ->where('context_id', (int) $this->deployment->getContext()->getId())
    ...
    {
        $this->deployment->addMetricRejection([
    ...
    {
        $contextId = (int) $this->deployment->getContext()->getId();
        $rors = [];
        foreach ($root->getElementsByTagNameNS($this->deployment->getNamespace(), 'institution_metric') as $node) {
    ...
    private function optionalReference(DOMElement $node, string $entity, string $attribute): ?int
    {
        ...
        return $this->deployment->requireReference($entity, $this->required($node, $attribute));
    }
