...
class PackageManifestTest extends TestCase
{
    ...
    {
        $directory = dirname(__DIR__, 2) . '/samples/full-journal-1.1';
        $manifest = PackageManifest::fromXml(
    ...
    public function testItRejectsIncompatibleManifestMetadata(): void
    {
        foreach ($this->incompatibleManifestProvider() as [$xml, $message]) {
            $this->assertIncompatibleManifestMetadata($xml, $message);
        }
    }

    private function assertIncompatibleManifestMetadata(string $xml, string $message): void
    {
        try {
            PackageManifest::fromXml($xml, self::RELEASE);
            $this->fail('Incompatible manifest metadata was accepted');
