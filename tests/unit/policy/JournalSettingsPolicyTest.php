...
class JournalSettingsPolicyTest extends TestCase
{
    ...
    public function testItRoundTripsEachTransportType(): void
    {
        foreach ($this->settingRoundTripProvider() as [$property, $value, $type, $payload]) {
            $this->assertTransportRoundTrip($property, $value, $type, $payload);
        }
    }

    private function assertTransportRoundTrip(string $property, $value, string $type, string $payload): void
    {
    ...
    public function testItRejectsUnknownSettingsAndInvalidTypesOrValues(): void
    {
        foreach ($this->invalidSettingProvider() as [$property, $value, $message]) {
            $this->assertInvalidSetting($property, $value, $message);
        }
    }

    private function assertInvalidSetting(
        string $property,
        $value,
        string $message
    ): void {
        try {
            ...
            $this->fail('Invalid context setting was accepted');
