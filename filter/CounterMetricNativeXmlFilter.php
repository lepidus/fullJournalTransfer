...
class CounterMetricNativeXmlFilter extends NativeExportFilter
{
    ...
    public function &process(&$metrics)
    {
        if (!$metrics instanceof \stdClass || !isset($metrics->daily, $metrics->monthly)) {
            throw new InvalidArgumentException('Expected COUNTER metrics for export');
        }
        $document = new DOMDocument('1.0', 'UTF-8');
        $root = $document->createElementNS('http://pkp.sfu.ca', 'counter_metrics');
        $document->appendChild($root);
        foreach (['daily' => $metrics->daily, 'monthly' => $metrics->monthly] as $granularity => $rows) {
            foreach ($rows as $metric) {
                $node = $document->createElementNS('http://pkp.sfu.ca', 'counter_metric');
